...
# -- figure: the SST responds (contrast with the fixed-SST example) ------
...
cf0 = axes[0].contourf(lon, lat, dTs, levels=np.linspace(-lim, lim, 17),
                       cmap='RdBu_r')
axes[0].set_title(f'surface temperature response to +{FORCING:.0f} '
                  f'W/m$^2$ with a slab ocean ({SLAB_YEARS} yr branch)')
...
fig.tight_layout()
fig.savefig('slab_co2_response.png', dpi=150)
print('wrote slab_co2_response.png')
